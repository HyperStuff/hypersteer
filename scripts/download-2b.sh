...
# Make sure the axbench/data directory exists
mkdir -p data
...
for (( i=START; i<=END; i++ )); do
    ...
    curl -L "${FILE_URL_16K}" -o data/gemma-2-2b_${i}-gemmascope-res-16k.json

    # Download the file for 65k resolution
    echo "Downloading ${FILE_URL_65K}..."
    curl -L "${FILE_URL_65K}" -o data/gemma-2-2b_${i}-gemmascope-res-65k.json
done
