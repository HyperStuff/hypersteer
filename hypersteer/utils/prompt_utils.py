...
async def get_concept_genres(client, concepts, api_tag=""):
    ...
    for i, response in enumerate(responses):
        if "none" in response.lower():
            concept_genres[concepts[i]] = [
                "text"
            ]  # if none, assign it with the text genre set
        else:
            genres = []
...
async def get_contrast_concepts(client, concepts, contrast_concepts=None, api_tag=""):
    ...
    # async step 2.
    prompts = [
        T_WORD_POLYSEMANTIC_MEANING.format(WORD=w, CONCEPT=concepts[i])
        for i, words in enumerate(all_words)
        for w in words
    ]
    flatten_words = [
        (w, concepts[i]) for i, words in enumerate(all_words) for w in words
    ]
    ...
    for _, ((w, concept), word_polysemantic) in enumerate(
        zip(flatten_words, word_polysemantics)
    ):
        if (
            "none" in word_polysemantic.lower()
            or w == ""
            or word_polysemantic == ""
            or len(word_polysemantic.split()) <= 1
        ):
            continue
        prompts += [
            T_FILTER_CONTRAST_CONCEPT.format(
    ...
    for i, polysemantic_check in enumerate(polysemantic_checks):
        ...
        if "yes" not in polysemantic_check:
            continue
        if contrast_concepts is not None and concept in contrast_concepts:
            existing_concepts = [item[-1] for item in contrast_concepts[concept]]
            if len(existing_concepts) != 0:
                prompts += [
                    T_FILTER_CONTRAST_MULTI_CONCEPT.format(
                        CONTRAST_CONCEPT=filtered_word_polysemantics[i][-1],
...
def get_random_content(
    seed_sentences, tokenizer, count, genres, concepts, length, split
):
...
async def modify_content_with_polysemantic_concepts(
    ...
    for i, polysemantic_concept in enumerate(polysemantic_concepts):
        prompts += [
            T_MODIFY_CONTENT_WITH_CONTRAST_CONCEPT.format(
                CONCEPT=polysemantic_concept[1],
                WORD=polysemantic_concept[0],
                CONTRAST_CONCEPT=concept,
                CONTENT=content[i],
            )
        ]
    responses = await client.chat_completions(
        f"{api_tag}.modify_content_with_polysemantic_concepts", prompts
    )
...
async def modify_content_with_concept(client, tokenizer, content, length, api_tag=""):
    ...
    for concept, tag, output in content:
        prompts.append(
            T_MODIFY_CONTENT_WITH_CONCEPT.format(CONTENT=output, CONCEPT=concept)
        )
    responses = await client.chat_completions(
        f"{api_tag}.modify_content_with_concept", prompts
    )
    pattern = re.compile(r"^(<[^>]*>\s*)+")
    return [
        tokenizer.convert_tokens_to_string(
            tokenizer.tokenize(pattern.sub("", response).strip(" .'").strip('"'))[
                : int(length)
            ]
        )
        for response in responses
...
async def continue_with(client, tokenizer, content, length, api_tag=""):
    ...
    for i, response in enumerate(responses):
        ...
        continued_tokens = full_tokens[
            content_token_lengths[i] : content_token_lengths[i] + int(length)
        ]
...
async def continue_with_concept(
    client, tokenizer, concepts, content, length, api_tag=""
):
    prompts = []
    content_token_lengths = []
    for i, c in enumerate(content):
        content_tokens = tokenizer.tokenize(c)
        content_token_lengths.append(len(content_tokens))
        prompts += [T_CONTINUE_WITH_CONCEPT.format(CONCEPT=concepts[i], CONTENT=c)]
    responses = await client.chat_completions(
        f"{api_tag}.continue_with_concept", prompts
    )
    continued_content = []
    for i, response in enumerate(responses):
        full_tokens = tokenizer.tokenize(response.strip(" '").strip('"'))
...
async def continue_without_concept(
    client, tokenizer, concepts, content, length, api_tag=""
):
    prompts = []
    content_token_lengths = []
    for i, c in enumerate(content):
        content_tokens = tokenizer.tokenize(c)
        content_token_lengths.append(len(content_tokens))
        prompts += [T_CONTINUE_WITHOUT_CONCEPT.format(CONTENT=c, CONCEPT=concepts[i])]
    responses = await client.chat_completions(
        f"{api_tag}.continue_without_concept", prompts
    )
    continued_content = []
    for i, response in enumerate(responses):
        full_tokens = tokenizer.tokenize(response.strip(" '").strip('"'))
...
async def continue_with_polysemantic_concepts(
    ...
    for i, polysemantic_concept in enumerate(polysemantic_concepts):
        prompts += [
            T_CONTINUE_WITH_CONTRAST_CONCEPT.format(
                CONCEPT=concept,
                WORD=polysemantic_concept[0],
    ...
    responses = await client.chat_completions(
        f"{api_tag}.continue_with_polysemantic_concepts", prompts
    )
    pattern = re.compile(r"^(<[^>]*>\s*)+")
    return (
        concept,
...
async def response_with_concept(
    client, tokenizer, concepts, content, length, api_tag=""
):
    prompts = []
    content_token_lengths = []
    for i, c in enumerate(content):
        content_tokens = tokenizer.tokenize(c)
        content_token_lengths.append(len(content_tokens))
        prompts += [T_RESPONSE_WITH_CONCEPT.format(INSTRUCTION=c, CONCEPT=concepts[i])]
    responses = await client.chat_completions(
        f"{api_tag}.response_with_concept", prompts
    )
    response_content = []
    for i, response in enumerate(responses):
        full_tokens = tokenizer.tokenize(response.strip(" '").strip('"'))
...
async def response_without_concept(
    client, tokenizer, concepts, content, length, api_tag=""
):
    prompts = []
    content_token_lengths = []
    for i, c in enumerate(content):
        ...
        content_token_lengths.append(len(content_tokens))
        prompts += [
            T_RESPONSE_WITHOUT_CONCEPT.format(INSTRUCTION=c, CONCEPT=concepts[i])
        ]
    responses = await client.chat_completions(
        f"{api_tag}.response_without_concept", prompts
    )
    response_content = []
    for i, response in enumerate(responses):
        full_tokens = tokenizer.tokenize(response.strip(" '").strip('"'))
...
async def response_with_polysemantic_concepts(
    ...
    for i, polysemantic_concept in enumerate(polysemantic_concepts):
        prompts += [
            T_RESPONSE_WITH_CONTRAST_CONCEPT.format(
    ...
    responses = await client.chat_completions(
        f"{api_tag}.response_with_polysemantic_concepts", prompts
    )
    pattern = re.compile(r"^(<[^>]*>\s*)+")
    return (
        concept,
