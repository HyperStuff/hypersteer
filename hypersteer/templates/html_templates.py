...
def get_valid_concept(data):
    for concept in (
        data["input_concept"] if "input_concept" in data else data["output_concept"]
    ):
        if concept and all(
            ord(c) < 128 for c in concept
...
def generate_html_with_highlight_text(data, lsreft_model="LsReFT"):
    rows = []
    ...
    for _, row in data.iterrows():
        ...
        reft_max_act = data[data["concept_id"] == concept_id][
            f"{lsreft_model}_max_act"
        ].max()
        ...
        reft_highlighted = [
            ...
            for token, act in zip(tokens, row[f"{lsreft_model}_acts"])
        ]
        ...
        rows.append(
            {
                ...
                if "input_concept" in row
                else row["output_concept"],
