...
@dataclass
class BaseModelCrossAttnOutputWithPast(BaseModelOutputWithPast):
...
class HypernetModel(HypernetPreTrainedModel):
    ...
    def __init__(self, config: Gemma2Config):
        ...
        self.layers = nn.ModuleList(
            [
                HypernetDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        base_encoder_hidden_states: torch.FloatTensor | None = None,
        base_encoder_attention_mask: torch.Tensor | None = None,
        base_encoder_position_ids: torch.LongTensor | None = None,
        ...
        if base_encoder_position_ids is None:
            base_encoder_position_ids = torch.arange(
                base_encoder_hidden_states.shape[1],
                device=base_encoder_hidden_states.device,
            ).unsqueeze(0)

        base_encoder_attention_mask = (
            self._update_encoder_attention_mask(
        ...
        for decoder_layer in self.layers:
            ...
            if output_attentions:
                self_attn, cross_attn = layer_outputs[1]
        ...
        return BaseModelCrossAttnOutputWithPast(
    ...
    def _update_encoder_attention_mask(
        ...
        dtype, device = (
            base_encoder_hidden_states.dtype,
            base_encoder_hidden_states.device,
        )
