...
class HypernetCrossAttention(Gemma2Attention):
    ...
    def __init__(self, config: Gemma2Config, layer_idx: int | None = None):
...
class HypernetDecoderLayer(Gemma2DecoderLayer):
    def __init__(self, config: Gemma2Config, layer_idx: int):
        ...
        self.cross_attention = HypernetCrossAttention(
            config=config, layer_idx=layer_idx
        )
        self.pre_cross_attention_layernorm = Gemma2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_cross_attention_layernorm = Gemma2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
