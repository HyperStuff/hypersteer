...
class WinRateEvaluator(Evaluator):
    ...
    def __init__(self, model_name, **kwargs):
        ...
        self.winrate_baseline = kwargs.get("winrate_baseline", "PromptSteering")
    ...
    def _get_all_ratings_from_data(self, data, column_name):
        ...
        model_relevance_concept_ratings = self._get_ratings_from_prompts(
            model_relevance_concept_prompts, f"{column_name}_concept"
        )
        model_relevance_instruction_ratings = self._get_ratings_from_prompts(
            model_relevance_instruction_prompts, f"{column_name}_instruction"
        )
        model_fluency_ratings = self._get_ratings_from_prompts(
    ...
    def compute_metrics(self, data):
        ...
        (
            baseline_relevance_concept_ratings,
            baseline_relevance_instruction_ratings,
            baseline_fluency_ratings,
        ...
        for i in range(len(baseline_relevance_concept_ratings)):
            ...
            if (
                abs(baseline_score - model_score) < 1e-6
